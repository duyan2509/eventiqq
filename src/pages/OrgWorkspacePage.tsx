...
import { OrgCheckInsTab } from '../components/org/OrgCheckInsTab'

type Tab = 'events' | 'members' | 'invitations' | 'permissions' | 'payment' | 'analytics' | 'checkins'
...
const NAV: NavItem[] = [
  ...
  { key: 'checkins', label: 'Check-ins', icon: <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg> },
]
    ...
    <div className="flex h-[calc(100vh-57px)]">
      ...
      <aside className="flex w-64 flex-shrink-0 flex-col border-r border-slate-800/60 bg-slate-950/40">
        ...
        <nav className="flex-1 overflow-y-auto space-y-0.5 p-2">
          {NAV.filter(item => !isStaffRole || item.key === 'events' || item.key === 'members' || item.key === 'checkins').map(item => (
            <button
              key={item.key}
